...
**チェック項目**:
- [ ] **各スライド 40 字以内・4 行以内**（★最重要、画像視認性ガード）
- [ ] **1 画像 1 メッセージを徹底**（情報量超過時は分割されているか）
- [ ] 役割（挑発・問題提起・証拠・思想爆発・正解・まとめ表・CTA）が揃っているか
- [ ] サイズが「中」「小」のみで「大」未使用か
- [ ] キャプション 3500 字以上
- [ ] Notion カルーセルページ保存完了
...
- 1 画像 1 メッセージで分割されている（枚数は情報量で決まる、固定なし）
...
**シナリオ**: TOP5 提示直後、ユーザー応答前に Phase 2 が起動しようとする
...
- ランキング下位に落ちて TOP5 入りしない
- 万が一 TOP5 入りしてしまった場合、ユーザー選択時に警告表示
...
1. ✅ 採用タイトル（カルーセル微調整版、13-25 字）
2. ✅ TOP5 ランキング（5 軸スコアと「この案の良さ／弱点」付き）
