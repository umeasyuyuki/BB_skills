...
5. 発見解剖系（discovery） 異常な事例を科学で分解（大谷の卵16個等・10枚固定）
```
...
Agent 1（並列起動）:
  ...
  prompt: |
    ...
    ★絶対遵守★
    - 台本の各スライドは必ずテキスト形式で出力すること。テーブル形式（| レイヤー | テキスト | サイズ | 色 |）は絶対NG。
    - 正しい形式: 「メイン・大・赤」をヘッダー行に書き、次の行からテキストを続ける。
    - 各スライドに感情誘発4型（裏切り/桁スケール/対比/自分事化）のいずれか1型以上を必ず組み込むこと。
    - キャプションは3,500字以上必須。Markdown記号（**太字**、##、—、／）を本文に残さないこと。
...
**★キャプション保存ガード（必須）:** カルーセルページの保存前に、キャプション（3,500字以上）がページ本文に含まれているか必ず確認する。含まれていない場合は保存を中断し、carousel-writer の出力からキャプションを取得して追加してから保存する。
...
4. **Phase 3 レンダリング実行**
   ...
   ```
   - 画像は `$BB_IMAGE_DIR/<slug>/` に出力される（環境変数未設定時は `./output/投稿画像/<slug>/`）
   - 成功/失敗の枚数を報告

5. **出力先を開く提案**
   ```
   画像生成完了。N枚のPNGが生成されました。
   Finder/Explorer で確認しますか？ → `open $BB_IMAGE_DIR/<slug>/`
   ```
